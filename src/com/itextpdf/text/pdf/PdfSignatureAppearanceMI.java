 ...
 * a signature.
 */
public class PdfSignatureAppearanceMI {
     ...
     */
    PdfSignatureAppearanceMI(PdfStamperImp writer) {
        this.writer = writer;
    ...
    private float[] cmPosition = null;
    private float[] ctPosition = null;

    public void setCheckMark(boolean enable, float[] checkMark) {
        this.checkMark = enable;
        this.cmPosition = checkMark;
    }

    public void setCheckText(boolean enable, float[] checkText) {
        this.checkText = enable;
        this.ctPosition = checkText;
    }
     ...
     */
    public PdfStamperMI getStamper() {
        return stamper;
    }
     ...
     */
    void setStamper(PdfStamperMI stamper) {
     ...
     * <p>
     * <strong>Be aware:</strong> if a signature is created on an existing
     * signature field, then its /Lock dictionary takes the precedence (if it
     * exists).</p>
     *
    ...
    private List<SignaturePosition> sigPosList = new ArrayList<>();

    public void setSigPosList(List<SignaturePosition> sigPosList) {
        this.sigPosList = sigPosList;
    }
    ...
    public void preClose(HashMap<PdfName, Integer> exclusionSizes) throws IOException, DocumentException {
        ...
        if (fieldExists) {
            ...
            if (pagesForInitials == null) {
                ...
                for (SignaturePosition sigPos : sigPosList) {
                    Rectangle rec = sigPos.getRectangle();
                    PdfFormField sigField = PdfFormField.createSignature(writer);
                    sigField.setFieldName(name + "_" + sigPos.getPage() + rec.getLeft() + rec.getBottom() + rec.getRight() + rec.getTop());
                    sigField.put(PdfName.V, refSig);
                    sigField.setFlags(PdfAnnotation.FLAGS_PRINT | PdfAnnotation.FLAGS_LOCKED);
                    ...
                    if (!isInvisible()) {
                        sigField.setWidget(sigPos.getRectangle(), null);
                    } else {
                        sigField.setWidget(new Rectangle(0, 0), null);
                    }
                ...
                sigField.setAppearance(PdfAnnotation.APPEARANCE_NORMAL, getAppearance());
                sigField.setPage(pagesForInitials[0]);
